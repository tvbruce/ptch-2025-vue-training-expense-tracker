    ...
    <div class="expense-chart">
        ...
        <div v-if="chartData && !loading" class="chart-stats">
            <div class="stat-item">
                ...
                <span class="stat-value expense">{{ formatCurrency(totalExpense) }}</span>
            </div>
            <div class="stat-item">
            ...
            <div class="stat-item">
                <span class="stat-label">最高支出</span>
                <span class="stat-value">{{ formatCurrency(maxExpense) }}</span>
            </div>
        </div>
    </div>
</template>
...
const chartData = computed(() => {
    const transactions = transactionStore.transactions.filter(t => t.type === 'expense')
...
const averageExpense = computed(() => {
    ...
    return totalExpense.value / chartData.value.datasets[0].data.length
})
                ...
                tooltip: {
                    ...
                    callbacks: {
                        label: function (context) {
                            return `支出：${formatCurrency(context.parsed.y)}`
                        }
                    }
                }
            },
